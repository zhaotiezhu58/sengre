package com.minghui.api.aop;

import cn.hutool.core.date.DateUtil;
import cn.hutool.json.JSONUtil;
import com.auth0.jwt.interfaces.DecodedJWT;
import com.minghui.api.utils.JwtUtils;
import com.minghui.commons.enums.ResultEnum;
...
public class LoginAspect {
    ...
    @Pointcut("execution(public * com.minghui.api.controller.*.*(..))")
    ...
    public Object login(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // token续期
        //redisTemplate.expireAt(tokenKey, DateUtil.offsetMinute(new Date(), 15));
        redisTemplate.expireAt(tokenKey, DateUtil.offsetDay(new Date(), 1));

        String onlineKey = RedisKeyUtil.UserOnlineKey(jwtUserPhone);
        if (redisTemplate.hasKey(onlineKey)) {
            redisTemplate.expireAt(onlineKey, DateUtil.offsetMinute(new Date(), 15));
        } else {
            redisTemplate.opsForValue().set(onlineKey, "online", 15, TimeUnit.MINUTES);
        }
        return joinPoint.proceed();
    }
